...
export const useUserType = () => {
  ...
  useEffect(() => {
    ...
    // Load last viewed dashboard from AsyncStorage
    const loadLastViewed = async () => {
      try {
        const lastViewed = await AsyncStorage.getItem('lastViewedDashboard');
        if (lastViewed === 'artist' || lastViewed === 'label' || lastViewed === 'agency') {
          setCurrentViewingTier(lastViewed);
        }
      } catch (error) {
        console.warn('Error reading last viewed dashboard from AsyncStorage', error);
      }
    };
  ...
  const switchDashboard = async (tier: 'artist' | 'label' | 'agency') => {
    if (!accessibleTiers.includes(tier)) {
      Alert.alert('Access Denied', `You don't have access to ${tier} dashboard`);
      return;
    }

    setCurrentViewingTier(tier);
    try {
      await AsyncStorage.setItem('lastViewedDashboard', tier);
    } catch (error) {
       console.warn('Error saving last viewed dashboard to AsyncStorage', error);
    }
    Alert.alert('Success', `Switched to ${tier} dashboard`);
  };
  ...
  const switchUserType = async (newType: 'artist' | 'label' | 'agency' | 'admin') => {
    ...
    if (newType === 'admin') {
      Alert.alert('Notice', 'Admin access is managed separately');
      return;
    }
